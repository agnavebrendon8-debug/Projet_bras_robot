import sys
import numpy as np
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QSlider, QLabel, QComboBox, QDoubleSpinBox, QPushButton, QGridLayout, QGroupBox, QStackedWidget, QStatusBar , QFileDialog
...
class RobotControlGUI(QMainWindow):
    ...
    def on_simulation_finished(self, result):
        ...
        if result["valid"]:
            ...
            # Lancement du mouvement du robot réel
            self.calculate_only = False
            self.updateSlider() 
...
# =======================================================================
    ...
    def process_and_buffer_gcode(self, file_path):
        """Traite le G-Code ligne par ligne avec Warm-Start et matrice 4x4 conforme"""
        try:
            ...
            # 1. FIX TECHNIQUE : Extraction correcte des coordonnées scalaires de la matrice MGD
            ...
            # Extraction des scalaires (mm) dans la 4ème colonne (index 3)
            last_x = float(T_current[0, 3])
            last_y = float(T_current[1, 3])
            last_z = float(T_current[2, 3])
            
            # Orientations actuelles par défaut (en radians)
            pitch = np.deg2rad(float(self.spins['Pitch'].value()))
            roll = np.deg2rad(float(self.spins['Roll'].value()))
            ...
            with open(file_path, 'r') as f:
                for line in f:
                    ...
                    if line.startswith('G1') or line.startswith('G0'):
                        ...
                        # Extraction des coordonnées textuelles
                        for part in parts:
                        ...
                        for k in range(pas_segment):
                            ...
                            T_step = np.array([
                                ...
                                [0.0,  sp,       cp,       inter_z],
                                [0.0,  0.0,      0.0,      1.0] # La 4ème ligne indispensable !
                            ], dtype=np.float64)
                            
                            # MGI Warm-Start ultra-rapide (repart de la ligne d'avant)
                        ...
                        # Sauvegarde pour le segment suivant
                        last_x, last_y, last_z = target_x, target_y, target_z
                        
                        # --- ENPÊCHE LE GEL ET LE CRASH DE LA FENÊTRE ---
                        # Force PyQt à traiter les événements graphiques (Webcam/IHM) pour ne pas figer
                        QApplication.processEvents()

            self.statusBar.showMessage(f"✅ G-Code compilé avec succès : {len(self.trajectory_buffer)} étapes prêtes.")
            
            # Configuration dynamique du bouton principal
            ...
            self.statusBar.showMessage(f"❌ Échec de la compilation : {str(e)}")
            print(f"Erreur G-code critique : {e}")
