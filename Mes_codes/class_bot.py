...
def update(frame):
    ...
    tab_angle.append(Chaine1.inverseCinematic3([target_x[frame] , target_y[frame] , np.abs(200*np.sin(frame * np.pi/(Frames))) ] , 4 , 0))
    t1 = Chaine1.Update( frame , tab_angle , motors)

    points.set_data([target_x[frame]], [target_y[frame]])
    points.set_3d_properties([target_z[frame]])
