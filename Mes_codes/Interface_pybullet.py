...
class RobotViewer(QObject):
    ...
    def check_pybullet_mouse_movement(self):
        """Lit la souris PyBullet uniquement si le Mode Simulation est coché sur l'IHM"""
        if self.robot is None or self.gui is None:
            return
            
        p.stepSimulation() 
        
        # --- FIX DESIGN INDUSTRIEL : On n'écoute la souris QUE si le mode 1 (PyBullet) est sélectionné ---
        if not hasattr(self.gui, 'control_mode') or self.gui.control_mode != 1:
            return # Si on est en mode IHM (0), PyBullet reste muet et reçoit juste les ordres
        ...
        # Synchronisation directe et forcée de l'IHM (Bande morte inutile ici car pas de conflit)
        if len(angles_deg) == 5:
            for idx, slider in enumerate(self.gui.sliders):
                slider.blockSignals(True)
                slider.setFloatValue(angles_deg[idx])
                self.gui.angle_labels[idx].setText(f"{angles_deg[idx]:.2f}°")
                slider.blockSignals(False)
            
            # Force la GUI à recalculer sa position cartésienne (MGD)
    ...
    def update_Simulation(self, angles_deg):
        ...
        # On active le verrou pendant le déplacement mécanique
    ...
    # --- FIX 2 : Libération immédiate du verrou une fois l'ordre transmis ---
    ...
    def simulate_motion(self, target_angles_deg):
        ...
        if hasattr(self, 'is_animating'): self.gui.is_animating = True 
